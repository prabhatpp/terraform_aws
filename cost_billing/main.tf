...
resource "aws_iam_role" "cost1_utilization_report_role" {
  name               = "cost1_utilization_report_role"
...
resource "aws_iam_role_policy" "cost_utilization_report_policy" {
  ...
  role   = aws_iam_role.cost1_utilization_report_role.id
  policy = data.template_file.policy.rendered
}
resource "aws_s3_bucket" "cost_utilization_s3" {
  bucket = "prab-cost-utilization-s3"
...
resource "aws_lambda_function" "cost_utilization_lambda" {
  ...
  role          = aws_iam_role.cost1_utilization_report_role.arn
...
resource "aws_cloudwatch_event_rule" "last_month" {
  name                = "last.Month"
...
resource "aws_cloudwatch_event_target" "last_day_of_the_month_target" {
  rule      = aws_cloudwatch_event_rule.last_month.name
...
resource "aws_lambda_permission" "allow_execution_of_target" {
  ...
  source_arn    = aws_cloudwatch_event_rule.last_month.arn
}